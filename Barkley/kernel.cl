...
__kernel void kBarkleyInterop(global float2* in,  global float2* out,
    float a1, float ba, float dt, float dte, float dth2, write_only image2d_t texOut) {
  int x = get_global_id(0),  y = get_global_id(1),  a = x + (y<<sh);
  if (x >= m || y >= m) return;
  ...
  out[a] = (float2)(unew, vnew);
    ...
    float4 pixel = (float4) (unew, vnew, 0, 255);
    write_imagef(texOut, (int2)(x,y), pixel);
}
