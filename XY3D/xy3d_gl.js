...
var gl, canvas, c_w, c_h, prog, tex_gl, mvMat, mvMatLoc, rotMat,
    transl = -2,  xOffs = yOffs = 0,  drag  = 0,  xRot = yRot = 0;
function init_gl() {
   c_w = window.innerWidth - 50;   c_h = window.innerHeight - 10;
   canvas.width = c_w;   canvas.height = c_h;
   var err = "Your browser does not support ";
   if (!window.WebGLRenderingContext){ alert(err+"WebGL");  return;}
   ...
   for ( var z = 0; z < n; z++ )
    for ( var y = 0; y < n; y++ )
      for ( var x = 0; x < n; x++ )  vertices.push( x/n, y/n, z/n );
