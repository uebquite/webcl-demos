...
__kernel void kAdvecInterop(global float2* U, global float* D,
    global float2* U1, global float* D1, write_only image2d_t texOut) {
    const float dt = .001f, tau = .5f*dt/h;
    ...
    int rg = (int)(255.0f*(1.0f + D[x + ny]));
    float4 bPix = (float4)( (uchar)clamp(rg, 0,255), (uchar)clamp(rg, 0,255),
        (uchar)clamp(255 + rg, 0,255), (uchar)255 );

    bPix.x /= 255; bPix.y /= 255;
    bPix.z /= 255; bPix.w /= 255;
    write_imagef(texOut, (int2)(x,y), bPix);
}
